...
export class ViewResetter extends Widget {
	...
	static type = new NodeType('ViewResetter', 'view-resetter', 
		Widget.type, ViewResetter);
	...
	constructor(name: string, parent: Node, data?: any) {
		...
		this.width.value = 80; this.height.value = 100;
		...
		let circle = new Component('circle', this.component, { 
			...
			new Component('text', this.component, 
			{ x: 40, y: 100,  text_anchor: 'middle', font_family: 'Arial', 
			font_size: 14, fill: 'url(#foreground_color)'}, 'Reset View');
	
		// Create the animation of the switch
		circle.animations['switch'] = new Animation(
			circle, 'r', 0, 20, 0, 0.25, false);

		// Create an event for the change
		function changed() { 
			circle.animations['switch'].play(1);
			if (this._onChange) this._onChange(this);
			let ui = (this.ancestor(UserInterface) as UserInterface)
			for (let callback of ui.onViewReset) callback();
		}
