...
export class Widget extends Node {
	...
	static type = new NodeType('Widget', 'widget', Node.type, Widget);


	// --------------------------------------------------------------- METADATA

	/** The valid values for the anchor property. */
	static anchorValues = ['top-left', 'top', 'top-right', 'left', 'center',
		'right', 'bottom-left', 'bottom','bottom-right', 'movable'];


	/** The valid values for the pivot property. */
	static pivotValues = ['top-left', 'top', 'top-right', 'left', 'center',
	...
	/** The parent component of the Widget. */
	protected _parentComponent: Component;

	/** The background component of the Widget. */
	protected _backgroundComponent: Component;

	/** The background color of the Widget. */
	protected _backgroundColor: Color;

	/** The background radius of the Widget. */
	protected _backgroundRadius: Number;

	/** The debug component of the Widget. */
	protected _debugComponent: Component;
	...
	protected _widgets: NodeSet<Widget>;

	/** Indicates if the events have been enabled. */
	protected _dragEvents : boolean; 

	/** The offset of the Widget in the X axis. */
	protected _offSetX: number

	/** The offset of the Widget in the Y axis. */
	...
	/** The pivot of the widget. */
	get pivot(): String { return this._pivot; }
	...
	constructor(name: string, parent: Node, data?: any,
		...
		this._anchor = new String('anchor', this,
			{ validValues: Widget.anchorValues, default: 'top-left' });
		this._pivot = new String('pivot', this,
			{ validValues: Widget.pivotValues, default: 'top-left' });
		this._position = new Vector('position', this);
		this._scale = new Number('scale', this, undefined, 1, 0.1, 10);
		this._widgets = new NodeSet<Widget>('widgets', this, Widget.type);
		...
		while (node) { 
			let component = (node as any).component
			if (component) { this._parentComponent = component; break }
			node = node.parent;
		}
	...
	update(forced: boolean = false): boolean {
		...
		let position = this.position.getValues(), scale = this._scale.value,
			x = position.x, y = position.y, 
			w = this._width.value || 0, h = this._height.value || 0;
		
		// If there is a pivot, calculate the offset
		this._offSetX = 0; this._offSetY = 0;
		switch (this._pivot.value) {
			case 'top': case 'center': case 'bottom': this._offSetX = w/2; break;
			case 'top-right': case 'right': case 'bottom-right': this._offSetX = w; break;
		}
		switch (this._pivot.value) {
			case 'left': case 'center': case 'right': this._offSetY = h/2; break;
			case 'bottom-left': case 'bottom': case 'bottom-right': this._offSetY = h; break;
		}
		if (this._offSetX != 0) x += this._offSetX; 
		if (this._offSetY != 0) y += this._offSetY;

		// If there is an anchor, user to the parent widget properties
		if (!this._anchor.isDefault) {
			let p = this.ancestor(Widget) as Widget, pw =0, ph =0;
			if (p) { pw=p.width.value; ph=p.height.value; x-=p._offSetX; y-=p._offSetY; }

			switch (this._anchor.value) {
				case 'top': case 'center': case 'bottom': x += (pw-w)/2; break;
				case 'top-right': case 'right': case 'bottom-right': x += pw-w; break;
			}
			switch (this._anchor.value) {
				case 'left': case 'center': case 'right': y += (ph-h)/2; break;
				case 'bottom-left': case 'bottom': case 'bottom-right': y+=ph-h; break;
			}
			if (this._anchor.value == 'movable') {
				if (!this._dragEvents) this.enableDragEvents();
			}
		}
		...
		else this._component.setAttribute('transform', 
		...
		} else this._backgroundComponent.setAttribute('fill', 'none');
	...
	resize(width?: number, height?: number) {
		...
		if (!this._width.isUndefined) width = this._width.value;
		if (!this._height.isUndefined) height = this._height.value;

		this._component.width = width; this._component.height = height;
		this._width.default = width; this._height.default = height; 

		
		// Resize the child widgets
		for (let child of this.widgets) child.resize(width, height);
		...
		switch (this._pivot.value) {
			case 'top': case 'center': case 'bottom':
				x1 -= width/2; x2 -= width/2; break;
			case 'top-right': case 'right': case 'bottom-right':
				x1 -= width; x2 -= width; break;
		}
		switch (this._pivot.value) {
			case 'left': case 'center': case 'right':
				y1 -= height/2; y2 -= height/2; break;
			case 'bottom-left': case 'bottom': case 'bottom-right':
				y1 -= height; y2 -= height; break;
		}

		// In the debug mode, create a grid
		if (this.debug) {
			let cellSize = 100;
			if (!this._debugComponent) this._debugComponent = new Component(
				'g', this._component, { id: 'debug', fill: 'none' });
			this._debugComponent.clear();
			new Component('rect', this._debugComponent, { x: x1, y: y1, 
				width: width, height: height, stroke: 'grey' });
			let x1c = Math.ceil(x1/cellSize) * cellSize,
				x2c = Math.floor(x2/cellSize) * cellSize,
				y1c = Math.ceil(y1/cellSize) * cellSize,
				y2c = Math.floor(y2/cellSize) * cellSize;
	...
	enableDragEvents() {
		let ui = this.ancestor<UserInterface>(UserInterface);
		let element = ui.component.element; if (!element) return;
		element.addEventListener('mousedown', (e:MouseEvent) => {
			this.react('down', e.buttons, e.offsetX, e.offsetY, 
				e.movementX, e.movementY);
			e.preventDefault();
		});
		window.addEventListener('mousemove', (e:MouseEvent) => {
			this.react('move', e.buttons, e.offsetX, e.offsetY, 
				e.movementX, e.movementY);
			e.preventDefault();
		});
		element.addEventListener('mouseup', (e:MouseEvent) => {
			this.react('up', e.buttons, e.offsetX, e.offsetY, 
				e.movementX, e.movementY);
			e.preventDefault();
		});
		element.addEventListener('wheel', (e:WheelEvent) => {
			this.react('zoom', e.buttons, 
				e.pageX - element.clientLeft, 
				e.pageY - element.clientTop, 
				e.movementX, e.movementY, e.deltaY < 0? 0.1: -0.1);
			e.preventDefault();
		});
		...
		element.addEventListener('touchend', (e:TouchEvent) => {
			let touch = e.touches[0], touchCount = e.touches.length,
				mx = touch.clientX - cursorX, my = touch.clientY - cursorY;
			cursorX = touch.clientX; cursorY = touch.clientY;
			this.react('up', touchCount, cursorX, cursorY, mx, my);
		});
		let touchSeparation: number = undefined;
		element.addEventListener('touchmove', (e:TouchEvent) => {
			if (e.touches.length < 2) {
				...
				let dx = e.touches[1].pageX - e.touches[0].pageX,
					dy = e.touches[1].pageY - e.touches[0].pageY,
					s = Math.sqrt(dx * dx + dy * dy) / 100,
					x = e.touches[1].pageX - element.clientLeft - dx/2, 
					y = e.touches[1].pageY - element.clientTop  - dy/2;
	...
	react(action: string, button: number, x: number, y: number,
		mx?: number, my?: number, mz?: number) {

		if (!this._enabled) return;
		if (action == 'move') {
			if (button > 0) this._position.addValues(mx, my); return;
		} 
		if (action == 'zoom') {
			// Prevent the zoom from being 0 or too 
			let oldZoom = this._scale.value, newZoom = this._scale.value + mz;
			if (newZoom < this._scale.min) newZoom = this._scale.min;
			else if (newZoom > this._scale.max) newZoom = this._scale.max;
			this._scale.value = newZoom;

			// Calculate the relative target
			let relX = -(this._position.x.value + this._offSetX),
				relY = -(this._position.y.value + this._offSetY);
			
			// Get the old and new targets in the 2D space
			let oldX = (x + relX) * oldZoom, oldY = (y + relY) * oldZoom,
				newX = (x + relX) * newZoom, newY = (y + relY) * newZoom;
